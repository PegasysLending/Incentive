...
describe('Enable incentives in target assets', () => {
  ...
  */
  before(async () => {
    ...
    await (
      await selfDestructContractV2.destroyAndTransfer(PEGASYS_GOVERNANCE_V2, {
        value: ethers.utils.parseEther('1'),
      })
    ).wait();
    ...
    await (
      await selfDestructContractV3.destroyAndTransfer(PEGASYS_SHORT_EXECUTOR, {
        value: ethers.utils.parseEther('1'),
      })
    ).wait();
  ...
  it('Proposal should be executed', async () => {
    const impersonatedGovernance = await ethers.provider.getSigner(PEGASYS_GOVERNANCE_V2);
    const impersonateExecutor = await ethers.provider.getSigner(PEGASYS_SHORT_EXECUTOR);
