...
makeSuite('pullRewardsIncentivesController claimRewardsToSelf tests', (testEnv) => {
  for (const {
    ...
    it(caseName, async () => {
      ...
      const { pullRewardsIncentivesController, pegasysToken, aDaiBaseMock } = testEnv;
      ...
      const destinationAddressBalanceBefore = await pegasysToken.balanceOf(destinationAddress);
      ...
      const destinationAddressBalanceAfter = await pegasysToken.balanceOf(destinationAddress);
